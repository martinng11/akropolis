import {
  ...
  SavingsModuleContract,
  SavingsModuleInstance,
  SavingsModuleOldContract,
  SavingsModuleOldInstance,
  RewardDistributionModuleContract,
  RewardDistributionModuleInstance,
  RewardVestingModuleContract,
  RewardVestingModuleInstance,
  PoolTokenContract,
  PoolTokenInstance,
  PoolTokenOldContract,
  PoolTokenOldInstance,
  StakingPoolContract,
  StakingPoolInstance,
  StakingPoolAdelContract,
  StakingPoolAdelInstance,
  FreeErc20Contract,
  FreeErc20Instance,
  YTokenStubContract,
  YTokenStubInstance,
...
contract(
  'Upgrades: migrate rewards from Savings to RewardDistribution',
  async ([owner, user, ...otherAccounts]) => {
    //let snap:Snapshot;
    ...
    before(async () => {
      ...
      stakingPoolAkro = await deployProxy(
        StakingPool,
        [pool.address, akro.address, '0'],
        UPGRADABLE_OPTS
      );
      await pool.set('staking', stakingPoolAkro.address, false);
      stakingPoolAdel = await deployProxy(
        StakingPoolADEL,
        [pool.address, adel.address, '0'],
        UPGRADABLE_OPTS
      );
