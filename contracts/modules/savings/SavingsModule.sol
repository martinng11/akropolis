    ...
    CapperRole
{
    ...
    function setDefaultUserCap(address _protocol, uint256 cap)
        public
        onlyCapper
    {
    ...
    function setWithdrawAllSlippage(address _protocol, uint256 slippageWei)
        public
        onlyOwner
    {
    ...
    function registerProtocol(IDefiProtocol protocol, PoolToken poolToken)
        public
        onlyOwner
    {
        uint256 i;
        for (i = 0; i < registeredProtocols.length; i++) {
            if (address(registeredProtocols[i]) == address(protocol))
                revert("SavingsModule: protocol already registered");
        }
        ...
        for (
            i = 0;
            i < protocols[address(protocol)].supportedRewardTokens.length;
            i++
        ) {
            address rtkn = protocols[address(protocol)]
                .supportedRewardTokens[i];
        ...
        uint256 normalizedBalance = protocols[address(protocol)]
            .previousBalance;
     ...
     */
    function updateProtocolRewardTokens(IDefiProtocol protocol)
        public
        onlyOwner
    {
        ...
        emit ProtocolRegistered(
            address(protocol),
            poolTokenByProtocol(address(protocol))
        );
    ...
    function deposit(
        ...
        public
        operationAllowed(IAccessModule.Operation.Deposit)
        returns (uint256[] memory)
    {
        require(
            _protocols.length == _tokens.length &&
                _tokens.length == _dnAmounts.length,
            "SavingsModule: size of arrays does not match"
        );
        ...
        returns (uint256)
    {
        ...
        for (uint256 i = 0; i < _tokens.length; i++) {
            nAmount = nAmount.add(
                normalizeTokenAmount(_tokens[i], _dnAmounts[i])
            );
        }
        ...
        if (protocolCapEnabled) {
            if (
                !(vipUserEnabled &&
                    protocols[_protocol].isVipUser[_msgSender()])
            ) {
                uint256 ptTS = poolToken.totalSupply();
                require(
                    ptTS <= protocolCap[_protocol],
                    "SavingsModule: deposit exeeds protocols cap"
                );
            }
        }
        ...
        if (userCapEnabled) {
            ...
            require(
                cap >= nAmount.sub(fee),
                "SavingsModule: deposit exeeds user cap"
            );
    ...
    function depositToProtocol(
        ...
        require(
            _tokens.length == _dnAmounts.length,
            "SavingsModule: count of tokens does not match count of amounts"
        );
        for (uint256 i = 0; i < _tokens.length; i++) {
    ...
    function withdrawAll(address _protocol, uint256 nAmount)
        public
        operationAllowed(IAccessModule.Operation.Withdraw)
        returns (uint256)
    {
        //distributeRewardIfRequired(_protocol);
        ...
        uint256 nBalanceBefore = distributeYieldInternal(_protocol);
        withdrawFromProtocolProportionally(
            _msgSender(),
            IDefiProtocol(_protocol),
            nAmount,
            nBalanceBefore
        );
        uint256 nBalanceAfter = updateProtocolBalance(_protocol);
        ...
        if (nBalanceAfter.add(nAmount) > nBalanceBefore) {
            ...
            require(
                actualAmount.sub(nAmount) <=
                    protocols[_protocol].withdrawAllSlippage,
                "SavingsModule: withdrawal fee exeeds slippage"
            );
        }
        ...
        returns (uint256)
    {
        ...
        require(
            maxNAmount == 0 || actualAmount <= maxNAmount,
            "SavingsModule: provided maxNAmount is too low"
        );
    ...
    function userCap(address _protocol, address user)
        public
        view
        returns (uint256)
    {
    ...
    function isVipUser(address _protocol, address user)
        public
        view
        returns (bool)
    {
    ...
    function poolTokenByProtocol(address _protocol)
        public
        view
        returns (address)
    {
    ...
    function protocolByPoolToken(address _poolToken)
        public
        view
        returns (address)
    {
    ...
    function rewardTokensByProtocol(address _protocol)
        public
        view
        ...
        returns (address[] memory poolTokens)
    {
        ...
        for (uint256 i = 0; i < poolTokens.length; i++) {
            poolTokens[i] = address(
                protocols[address(registeredProtocols[i])].poolToken
            );
        }
    }
    ...
    function withdrawFromProtocolProportionally(
        address beneficiary,
    ...
    function withdrawFromProtocolOne(
        address beneficiary,
        IDefiProtocol protocol,
        address token,
        uint256 dnAmount
     ...
     */
    function distributeYieldInternal(address _protocol)
        internal
        returns (uint256)
    {
    ...
    function createYieldDistribution(PoolToken poolToken, uint256 yield)
        internal
    {
     ...
     * @return Current balance of the protocol
     */
    function updateProtocolBalance(address _protocol)
        internal
        returns (uint256)
    {
        uint256 currentBalance = IDefiProtocol(_protocol).normalizedBalance();
    ...
    function isPoolToken(address token) internal view returns (bool) {
        for (uint256 i = 0; i < registeredProtocols.length; i++) {
            ...
            if (address(protocols[address(protocol)].poolToken) == token)
                return true;
        }
        return false;
    }

    function normalizeTokenAmount(address token, uint256 amount)
        private
        view
        returns (uint256)
    {
    ...
    function denormalizeTokenAmount(address token, uint256 amount)
        private
        view
        returns (uint256)
    {
